	...
	return {
				...
				},
				{
					...
					use: [
						isDevMode ? 'style-loader' : MiniCssExtractPlugin.loader,
						{
							...
							options: {
								modules: {
									localIdentName: isDevMode ? "[path]-[name]--[local]" : "[hash:base64:5]",
		...
		plugins: [
			new webpack.DefinePlugin({
				...
				'process.env.APP_AUTHOR': JSON.stringify(require("../package.json").author),
			...
			...(!isLocalStand ? [new InjectManifest({
				swSrc: path.resolve(__dirname, '../src', 'serviceWorkerManifest.js'),
